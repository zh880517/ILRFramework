...
namespace SharpRecast
{
    public static class RecastGizmos
    {
        ...
        {
            Vector3 origin = new Vector3(gridData.OriginPoint.x, 0, gridData.OriginPoint.y);
            for (int x = 0; x < gridData.Width; ++x)
            {
                for (int z = 0; z < gridData.Length; ++z)
                {
                    ...
                    {
                        Vector3 size = new Vector3(1, 0.2f, 1)*gridData.CellSize;
                        Vector3 pos = new Vector3(x + 0.5f, 0, z + 0.5f)*gridData.CellSize + origin;
                        Gizmos.DrawCube(pos, size);
                    }
                }
            }
